...
class data_loader(object):
    ...
    def sample_icml(self, batch_size=1000, negative_count=10):
        ...
        if len(self.kb.facts_time_tokens) == 0:
            ...
            if self.perturb_time:
                for i in range(len(t)):
                    # print(self.kb.datamap.reverse_entity_map[facts[i][0]])
                    # print(self.kb.datamap.reverse_relation_map[facts[i][1]])
                    # print(self.kb.datamap.reverse_entity_map[facts[i][2]])
                    start, end = t[i, time_index["t_s_orig"], 0], t[i, time_index["t_e_orig"], 0]
                    try: 
                        t[i, time_index["t_s"], 0] = numpy.random.randint(low=start, high=end + 1)
                    except:
                        print(self.kb.datamap.reverse_entity_map[facts[i, 0]],self.kb.datamap.reverse_entity_map[facts[i, 2]]) 
                        print(self.kb.datamap.id2dateYear[start], self.kb.datamap.id2dateYear[end])
                        print(facts[i])
                        print(t[i, time_index["t_s_orig"], 0], t[i, time_index["t_e_orig"], 0])
                        print(start, end)
                        dsjq
        else:  # for TA-x model
            t = self.kb.facts_time_tokens[indexes]
