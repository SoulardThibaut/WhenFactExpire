...
class Datamap(object):
    ...
    """
    def __init__(self,dataset, dataset_root, time_precision, use_time_interval=False):
        self.dataset = dataset
        self.dataset_root = dataset_root
        self.use_time_interval=use_time_interval

        self.temporal_precision = time_precision
        ...
        with open(train_filename) as f:
            ...
            if self.use_time_interval and (len(self.year2id) == 0):
                ...
                for line in lines:
                    ...
                    count += 1
                self.year2id = self.create_date2id(triple_time, bin_size=300)  # (bin_start, bin_end) to id
            # ------- #
    ...
    def get_time_info(self, dataset_root=""):
        ...
        with open(os.path.join(dataset_root, "all_timestamps.txt"), "r", encoding="UTF-8") as f_r:
            line = f_r.readline()
            while line != "":
                date = self.check_date_validity(line[:-1])
                all_years.append(date)
                line = f_r.readline()
    ...
    def create_date2id(self, triple_time, bin_size=300):
        year2id = dict()
        freq = dd(int)
        count = 0
        year_list = []
        ...
        for k, v in triple_time.items():
            try:
                start = self.check_date_validity(v[0])
                end = self.check_date_validity(v[1])
    ...
    def check_date_validity(self, date):
        try:
            return numpy.datetime64(date, self.temporal_precision).astype(int) 
...
class kb(object):
                ...
                if len(l)==3: # for non-temporal datasets
                    ...
                    if self.datamap.use_time_interval:
                        if len(l) == 5:  # timestamp of the form "occursSince <YEAR>" or "<YEAR1> <YEAR2>"
                            ...
                            time_str = '\t'.join(l[3:])
                            if time_str not in self.datamap.timeStr2Id:
                                self.datamap.timeStr2Id[time_str] = len(self.datamap.timeStr2Id)
                                self.datamap.id2TimeStr[self.datamap.timeStr2Id[time_str]] = time_str
    ...
    def get_span_ids(self, start_in, end_in):
        ...
        start = self.datamap.check_date_validity(start)
        end = self.datamap.check_date_validity(end)
        if start > end:
            end = YEARMAX
