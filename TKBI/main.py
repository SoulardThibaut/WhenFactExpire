...
from time_prediction.evaluate import evaluate_homemade as time_evaluate_homemade
...
def main(mode, dataset, dataset_root, save_dir, tflogs_dir, debug, model_name, model_arguments,
    ...
    if resume_from_save:
        ...
        # build datamap
        datamap = kb.Datamap(dataset, dataset_root, time_precision, use_time_interval)
    ...
    datamap_filter = kb.Datamap(dataset, dataset_root_filter, time_precision,use_time_interval=False)
    ...
    if mode == 'train':
        ...
        print("post_training", flush=True)
        if resume_from_save:
            mb_start = tr.load_state(resume_from_save)
        else:
            mb_start = 0
        max_mini_batch_count = int(max_epochs * ktrain.facts.shape[0] / batch_size)
        print("max_mini_batch_count: %d, eval_batch_size %d" % (max_mini_batch_count, eval_batch_size),flush=True)
        tr.start(max_mini_batch_count, [-1, -1], mb_start, tflogs_dir, timeout=timeout
                 )
        ...
        valid_score = 0#evaluate.evaluate("valid", ranker, kvalid, eval_batch_size,
        ...
        #                         filter_method=filter_method, flag_additional_filter=flag_additional_filter,
        #                         expand_mode=expand_mode, load_to_gpu=has_cuda)
        test_score = 0#evaluate.evaluate("test", ranker, ktest, eval_batch_size,
                                    #    verbose=verbose, hooks=hooks, save_text=save_text,
        ...
        ktest_sub_valid_prediction = kb.kb(datamap, os.path.join(dataset_root, 'ValidationTask/test.txt'),
                          add_unknowns=int(not (int(introduce_oov))),
                          use_time_tokenizer=use_time_tokenizer)

        kvalid_sub_valid_prediction = kb.kb(datamap, os.path.join(dataset_root, 'ValidationTask/valid.txt'),
                           add_unknowns=int(not (int(introduce_oov))),
                           use_time_tokenizer=use_time_tokenizer)
        ...
        if predict_time:
            ...
            time_evaluate_homemade(scoring_function, kvalid_sub_valid_prediction, ktest_sub_valid_prediction, time_args=time_args, dump_t_scores=dump_t_scores,
                          load_to_gpu=has_cuda, save_time_results= save_time_results)
        # '''
        # ------------------- #
...
if __name__ == "__main__":
    ...
    parser.add_argument('--time_precision', required=True, choices=('Y', 'M', 'D'))
    # ------------------------------#
