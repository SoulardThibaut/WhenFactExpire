...
def get_time_scores(scoring_function, test_kb, method='greedy-coalescing', load_to_gpu=True):
    ...
    if method in ['greedy-coalescing']:  # for time-point models
        ...
        for i in range(0, int(facts.shape[0]), 1):
            ...
            s, r, o = fact[:3]
            # print(s,r,o)
            # print(test_kb.datamap.reverse_entity_map[s], test_kb.datamap.reverse_relation_map[r], test_kb.datamap.reverse_entity_map[o])
            ...
            s = numpy.repeat(s,1)# num_times)
            r = numpy.repeat(r,1)# num_times)
            o = numpy.repeat(o,1)# num_times)
...
def compute_interval_scores_homemade(valid_time_scores_dict, test_time_scores_dict, save_time_results=None,
                            method='greedy-coalescing', path_out=""):
    """
    Takes input time scores stored in test_pickle and valid_pickle (for test and valid KBs respectively)
    Using these time scores, depending on method it predicts intervals for each fact in test kb (a ranking ideally instead of a single interval),
    and returns gIOU/aeIOU/IOU scores @1/5/10
    ...
    # load from dict
    valid_t_scores, _, valid_facts, _, _, valid_id_year_map, valid_t_gold_min, valid_t_gold_max = load_pickle(
        valid_time_scores_dict)
    test_t_scores, _, test_facts, _, _, test_id_year_map, test_t_gold_min, test_t_gold_max = load_pickle(
        test_time_scores_dict)
    ...
    if method == 'greedy-coalescing':
        ...
        test_t_scores = stack_tensor_list(test_t_scores)
        valid_t_scores = stack_tensor_list(valid_t_scores)

        aggr = 'mean'
        thresholds_valid = get_thresholds(valid_t_scores, valid_facts, valid_facts, aggr=aggr)
        thresholds_test = get_thresholds(valid_t_scores, valid_facts, test_facts, aggr=aggr)
        print("Computed thresholds (aggr= {})\n".format(aggr))

    elif method in ['start-end-exhaustive-sweep']:  # for time boundary models
        start_t_scores, end_t_scores = test_t_scores

        start_t_scores = stack_tensor_list(start_t_scores)
        end_t_scores = stack_tensor_list(end_t_scores)

        test_t_scores = (start_t_scores, end_t_scores)

    # load valid scores

    # compute thresholds

    valid_id_year_map = valid_id_year_map.long()

    valid_id_year_map_dict = {}
    for i, j in enumerate(valid_id_year_map):
        valid_id_year_map_dict[i] = j

    test_id_year_map = test_id_year_map.long()

    test_id_year_map_dict = {}
    for i, j in enumerate(test_id_year_map):
        test_id_year_map_dict[i] = j
  ...
  #  valid_scores_dict = {}  # for saving later

   # for score_to_compute in score_func.keys():
   #     print("\nScore:{}".format(score_to_compute))
        # iou_scores= compute_scores(ktrain, facts, t_scores, method=method, durations=durations,
        # 			  score_func=score_func[score_to_compute], use_time_interval=use_time_interval, topk_ranks=topk_ranks)
  #      iou_scores = compute_scores(valid_id_year_map, valid_facts, valid_t_gold_min, valid_t_gold_max, valid_t_scores, method=method,
...
#            valid_scores_dict[(i, score_to_compute)] = best_scores

            #     print("best_scores shape:",best_scores.shape)
   #         print("Best {} @{}: {}".format(score_to_compute, i, torch.mean(best_scores)))

            # with open(f"{path_out}{score_to_compute}_at_{i}", "w") as f_out: 
            #     f_out.write(str(best_scores))

    test_scores_dict = {}  # for saving later

    for score_to_compute in score_func.keys():
        print("\nScore:{}".format(score_to_compute))
        # iou_scores= compute_scores(ktrain, facts, t_scores, method=method, durations=durations,
        # 			  score_func=score_func[score_to_compute], use_time_interval=use_time_interval, topk_ranks=topk_ranks)
        iou_scores = compute_scores(test_id_year_map, test_facts, test_t_gold_min, test_t_gold_max, test_t_scores, method=method,
        ...
        for i in iouatk:
            ...
            test_scores_dict[(i, score_to_compute)] = best_scores
    ...
    if save_time_results is not None:
        # saves metrics of the form
        #pickle_filename = "{}_valid_time_scores_analysis".format(save_time_results)

        #gold_min, gold_max, pred_min, pred_max = compute_preds(valid_id_year_map, valid_facts, valid_t_gold_min, valid_t_gold_max, valid_t_scores,
        #                                                       method=method, thresholds=thresholds_valid,
        #                                                       score_func=score_func[score_to_compute],
        #                                                       topk_ranks=topk_ranks)

        #with open(pickle_filename, 'wb') as handle:
        #    pickle.dump({'facts': valid_facts, 'scores_dict': valid_scores_dict, 'time_scores':valid_t_scores, 't_gold_min': gold_min, 't_gold_max': gold_max,
        #                 't_pred_min': pred_min, 't_pred_max': pred_max}, handle)
        #    print("\nPickled scores, t_gold_min, t_gold_max, t_pred_min, t_pred_max to {}\n".format(pickle_filename))

        pickle_filename = "{}_test_time_scores_analysis".format(save_time_results)

        gold_min, gold_max, pred_min, pred_max = compute_preds(test_id_year_map, test_facts, test_t_gold_min, test_t_gold_max, test_t_scores,
                                                               method=method, thresholds=thresholds_test,
                                                               score_func=score_func[score_to_compute],
                                                               topk_ranks=topk_ranks)

        with open(pickle_filename, 'wb') as handle:
            pickle.dump({'facts': test_facts, 'scores_dict': test_scores_dict, 'time_scores':test_t_scores, 't_gold_min': gold_min, 't_gold_max': gold_max,
                         't_pred_min': pred_min, 't_pred_max': pred_max}, handle)
            print("\nPickled scores, t_gold_min, t_gold_max, t_pred_min, t_pred_max to {}\n".format(pickle_filename))
...
def evaluate_homemade(scoring_function, valid_kb, test_kb, time_args, dump_t_scores=None, load_to_gpu=True, save_time_results=None, save_text=""):
    method = time_args['method']
    print(str(scoring_function))

    valid_time_scores_dict = get_time_scores(scoring_function, valid_kb, method, load_to_gpu)
    valid_time_scores_dict["FactsReadable"] = [(valid_kb.datamap.reverse_entity_map[f[0]], valid_kb.datamap.reverse_relation_map[f[1]], test_kb.datamap.reverse_entity_map[f[2]]) for f in valid_time_scores_dict["facts"]]
    test_time_scores_dict = get_time_scores(scoring_function, test_kb, method, load_to_gpu)
    test_time_scores_dict["FactsReadable"] = [(test_kb.datamap.reverse_entity_map[f[0]], test_kb.datamap.reverse_relation_map[f[1]], test_kb.datamap.reverse_entity_map[f[2]]) for f in test_time_scores_dict["facts"]]
    
    # pdb.set_trace()


    if method in ['greedy-coalescing', 'start-end-exhaustive-sweep']:
        compute_interval_scores_homemade(valid_time_scores_dict, test_time_scores_dict, method=method, save_time_results=save_time_results, path_out="./ResultsExpe/")
        # compute_interval_scores_homemade(valid_time_scores_dict, test_time_scores_dict, method=method, save_time_results=save_time_results, path_out="./ResultsExpe/")
    else:
        raise Exception("Not implemented")

    if dump_t_scores is not None:
      #  valid_pickle_filename = "./ResultsExpe/{}_t_scores_homemade_{}".format(
      #      dump_t_scores, 'valid')
      #  with open(valid_pickle_filename, 'wb') as handle:
      #      pickle.dump(valid_time_scores_dict, handle)
      #      print("Pickled t scores to {}\n".format(valid_pickle_filename))

        test_pickle_filename = "./ResultsExpe/{}_t_scores_homemade_{}".format(
            dump_t_scores, 'test')
        with open(test_pickle_filename, 'wb') as handle:
            pickle.dump(test_time_scores_dict, handle)
            print("Pickled t scores to {}\n".format(test_pickle_filename))
