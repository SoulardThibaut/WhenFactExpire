...
def string_to_year(date, default=YEAR_MAX):
    if type(date) != str:
        return date
    if date[0] == "-":
        if len(date[1:].split("-")) == 3: #Y-M-D
            year, month, day = date[1:].split('-')
            return (int(year)*375 + int(month)*31 + int(day))*-1

        elif len(date[1:].split("-")) == 2: # Y-M
        ...
        if len(date.split("-")) == 3: #Y-M-D
            year, month, day = date.split('-')
            return int(year)*375 + int(month)*31 + int(day)
        
        elif len(date.split("-")) == 2: #Y-M
            year, month = date.split('-')
            return int(year)*12 + int(month)
...
def get_gold_year_interval(t, id2_time_str, id_year_map, load_to_gpu=True, test_kb=None):
    ...
    for ele in t_str:
        t_gold_min_ele, t_gold_max_ele = id2_time_str[ele[0].item()].split(
            "\t")
        t_gold_min_ele = test_kb.datamap.check_date_validity(t_gold_min_ele)
        t_gold_max_ele = test_kb.datamap.check_date_validity(t_gold_max_ele)
...
def prepare_data_iou_scores(t, test_kb, scores_t="", load_to_gpu=True):
    ...
    t_gold_min, t_gold_max = get_gold_year_interval(
        t, id2_time_str, id_year_map, load_to_gpu=load_to_gpu, test_kb=test_kb)
    # ------------------------------------------#
    print(len(scores_t), scores_t[0])
