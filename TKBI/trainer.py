...
class Trainer(object):
    def __init__(self, scoring_function, scoring_function_arguments, regularizer, loss, optim, train, valid, test,
        ...
        print("Before ranker",flush=True)
    ...
    def start(self, steps=50, batch_count=(20, 10), mb_start=0, logs_dir="", predict_time=0,
              time_prediction_method='greedy-coalescing', predict_rel=0, timeout=-1):
        start = time.time()
        losses = []
        count = 0
        ...
        valid_score = {"e2":{"mrr":0},"e1":{"mrr":0}, "m":{"mrr":0}, "t":{"mrr":0}, "r":{"mrr":0}}
#evaluate.evaluate("valid", self.ranker_valid, self.valid.kb, self.eval_batch,
                       #                                 predict_rel=predict_rel, verbose=self.verbose, hooks=self.hooks, load_to_gpu=self.load_to_gpu, 
                       #                                 flag_add_reverse=self.flag_add_reverse)
        test_score = {"e2":{"mrr":0},"e1":{"mrr":0}, "m":{"mrr":0}, "t":{"mrr":0}, "r":{"mrr":0}}
#evaluate.evaluate("test ", self.ranker_test, self.test.kb, self.eval_batch,
                      #              predict_rel=predict_rel, verbose=self.verbose, hooks=self.hooks, load_to_gpu=self.load_to_gpu,
                      #              flag_add_reverse=self.flag_add_reverse)
        self.save_state(i, valid_score, test_score)
        print()
