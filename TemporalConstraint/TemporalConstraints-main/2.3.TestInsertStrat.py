...
if __name__ == '__main__':
    ...
    file_res = open(f"{path}Results_Naive_vs_Remove.csv", "w", encoding="UTF-8") 
    ...
    print(1.2)
    start = time.time()
    cmd = f"python3 ./1.2.RulesWeightsTrainRemove.py {type_ent} Rules_{strat}_{gt}_{vt}.tsv train_ML.quintuplet 19"
    check_call(cmd, shell=True, stdout=DEVNULL, stderr=STDOUT)
    end = time.time()
    f_time.write(f"{strat},{gt},{vt},Remove,1,{end-start}\n")

    start = time.time()
    cmd = f"python3 ./1.3.RulesWeightsTesterRemove.py {type_ent} Rules_{strat}_{gt}_{vt}.tsv valid.quintuplet 19"
    check_call(cmd, shell=True, stdout=DEVNULL, stderr=STDOUT)
    end = time.time()
