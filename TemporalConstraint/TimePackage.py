...
class Triple:
    ...
    def __init__(self, head:int, relation:int, value, date:Interval, is_object:bool=True) -> None:
        self.head = head
...
class TimeSequenceRelation:
    ...
    name_relation_A = None
    name_relation_B = None
    ...
    def __init__(self, name_relation_A, name_relation_B, seq_A, seq_B, constraint_to_check=None) -> None:
        ...
        if (not self.seq_A.multi_valuation_temporal) and (not self.seq_B.multi_valuation_temporal):
            self.at_least_one_multi = False
            ...
            self.at_least_one_multi = True
            #print("At least one of the two is multi valued", flush=True)
...
class TemporalRule:
    ...
    def __init__(self, a:int, precision_a:str, r:str, b:int, precision_b:str, error_percentage:float, coverage_percentage:float) -> None:
    ...
    def __hash__(self) -> int:
        res = f"{self.a}"
        if self.precision_a:
            res += f"X{self.precision_a}"
        res += f" {self.r} "
        res += f"{self.b}"
        if self.precision_b:
            res += f"X{self.precision_b}"
        return hash(res)
...
def ordered_time_sequence_first_start(entity:Entity, r:str, temporal_granularity:str="D"):
    triples_of_r = entity.get_triples_with_r(r)
...
def ordered_time_sequence_first_start_with_rxv(entity:Entity, r, temporal_granularity:str="D"):
...
def ordered_time_sequence_first_start_with_rxv_return_triples(entity:Entity, r, temporal_granularity:str="D"):
    if r in entity.triples_per_r_and_rxv:
        r_per_start = {}
        triples_of_r = entity.triples_per_r_and_rxv[r]
        for t in triples_of_r:
